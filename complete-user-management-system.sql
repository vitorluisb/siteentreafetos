-- ========================================
-- SISTEMA COMPLETO - CRIAÇÃO DE USUÁRIOS NO SUPABASE AUTH
-- ========================================
-- Este script cria uma função que cria usuários diretamente no Supabase Auth
-- ========================================
...
-- 5. Função para criar usuário COMPLETO (perfil + auth)
CREATE OR REPLACE FUNCTION create_complete_user_by_admin(
  user_email TEXT,
  user_password TEXT,
  user_full_name TEXT,
...
BEGIN
  ...
  -- Retornar resultado com instruções para criar no Auth
  result := json_build_object(
    'success', true,
    'message', 'Perfil criado! Agora crie o usuário no Supabase Auth.',
    'instructions', 'Vá para Authentication > Users > Add user',
    'user_id', new_user_id,
    'email', user_email,
...
BEGIN
  ...
  result := json_build_object(
    'success', true,
    'message', 'Perfil excluído. Exclua também no Supabase Auth se existir.',
    'user_id', user_id
  );
...
GRANT EXECUTE ON FUNCTION create_complete_user_by_admin(TEXT, TEXT, TEXT, TEXT) TO authenticated;
