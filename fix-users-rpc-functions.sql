-- ========================================
-- CORREÇÃO DAS FUNÇÕES RPC PARA GESTÃO DE USUÁRIOS
-- ========================================
-- Este script corrige os tipos de dados das funções RPC
-- ========================================
...
DROP FUNCTION IF EXISTS update_user_by_admin(UUID, TEXT, TEXT);
DROP FUNCTION IF EXISTS delete_user_by_admin(UUID);

-- 2. Função corrigida para listar usuários
...
BEGIN
  ...
  SELECT 
    u.id,
    u.email::VARCHAR(255),
    COALESCE(p.full_name, u.raw_user_meta_data->>'name', '')::VARCHAR(255) as full_name,
    COALESCE(p.display_name, u.raw_user_meta_data->>'display_name', u.raw_user_meta_data->>'name', '')::VARCHAR(255) as display_name,
    COALESCE(p.role, u.raw_user_meta_data->>'role', 'user')::VARCHAR(50) as role,
    u.created_at,
    u.last_sign_in_at,
...
-- 3. Função para criar usuário (versão simplificada)
...
BEGIN
  ...
  result := json_build_object(
    'success', true,
    'message', 'Perfil de usuário criado. Crie o usuário no Supabase Auth manualmente.',
    'user_id', new_user_id,
    'email', user_email,
...
-- 4. Função para atualizar usuário
...
BEGIN
  ...
  result := json_build_object(
    'success', true,
    'message', 'Perfil de usuário excluído. Exclua o usuário no Supabase Auth manualmente.',
    'user_id', user_id
  );
...
-- 6. Conceder permissões de execução para usuários autenticados
